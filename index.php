...
if (isset($_SERVER['HTTP_ORIGIN'])) {
	$allowed_origin = array('http://localhost:8080', $_SERVER["HTTP_ORIGIN"], 'https://localhost:8080', 'http://127.0.0.1:8080', 'https://127.0.0.1:8080');
...
try {
	...
	try {
		...
		$exclude_auth = array('login', 'get_module', 'cek_pelayanan', 'register');
		if (
			(
				(($requestTarget == 'PondokCoder\\Member') && $_SERVER['REQUEST_METHOD'] === 'POST') &&
            ...
            (
                (($requestTarget == 'PondokCoder\\Pelayanan') && $_SERVER['REQUEST_METHOD'] === 'POST') &&
                (in_array($ParameterBuilder['request'], $exclude_auth))
            )
		) {
			if(in_array($ParameterBuilder['request'], $exclude_auth)) {
				$ClassMethod = call_user_func_array('PondokCoder\\Member::__POST__', array($ParameterBuilder));
				echo json_encode($ClassMethod);
			} else {
			    if($requestTarget == 'PondokCoder\\Pelayanan') {
                    $ClassMethod = call_user_func_array('PondokCoder\\Pelayanan::__POST__', array($ParameterBuilder));
                    echo json_encode($ClassMethod);
                } else if($requestTarget == 'PondokCoder\\Master') {
                    $ClassMethod = call_user_func_array('PondokCoder\\Master::__POST__', array($ParameterBuilder));
                    echo json_encode($ClassMethod);
                } else {
